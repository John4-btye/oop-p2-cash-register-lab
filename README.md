...
4. When you are ready to submit, click the **_Load Lab: Object Oriented Programming (OOP)- Part 2- Cash Register_** button in Canvas to launch CodeGrade.

- Click on + Create Submission. Connect your repository for this lab.
- For additional information on submitting assignments in CodeGrade: [Getting Started in Canvas](https://help.codegrade.com/for-students/getting-started/getting-started-in-canvas).

# Project Overview (OOP Python Project)

This project implements a simple Cash Register system using object-oriented programming in Python. The class simulates basic functionality you would find in an e-commerce checkout system, including adding items, applying discounts, and voiding transactions.
...
- stores each item in a list (including duplicates based on quantity)

2. Apply discount

- applies a percentage-based discount to the total
- prints the updated total after discount
- handles edge cases:
  - no items in cart
  - discount is 0
...
3. Void last transaction
    ...
    * removes the correct number of items from the cart
    * handles case where no transactions exist
...
- items - list of all items added (includes duplicates)
- previous_transactions - stores transaction history
...
- prints: After the discount, the total comes to $<amount>.
- handles invalid cases
  void_last_transaction()
- removes the last transaction
- adjusts total and items list

# Key concepts used

- (OOP)
- Class design and instance attributes
- Method definitions and defualt paramters
- Input validation
- List manipulation
- Dictionary usage for transaction tracking

# Testing

This project was tested using pytest:

- All required tests pass
- Edge cases handled (no transactions, zero discount, multiple quantities)
- Output formatting matches expected results exactly
  Then run tests with: pytest -x
- pytest -x: will stop testing suite when 1 problem is encountered so that you can focus on one issue at a time.

# Final notes

- The items list stores each item individually based on quantity
- Discount logic uses percentage-based calculations
